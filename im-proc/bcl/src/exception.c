 ...
 * Copyright (C) 1996, 98, 99, 2002 Achille Braquelaire (achille@labri.fr)
 * 
...
/*
  Function used to emit an error message on uncaught exception
*/
...
static sigset_t exception_mask_svg;

void
exception_block_signals(void)
{
    static int first = 1;
    ...
    sigprocmask(SIG_SETMASK, &block_all_signals, &exception_mask_svg);
}

void 
exception_restaure_signals(void)
{
    sigprocmask(SIG_SETMASK, &exception_mask_svg, NULL);
}
