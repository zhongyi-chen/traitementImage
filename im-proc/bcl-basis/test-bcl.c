...
void process(const size_t rows, const size_t cols)
{
  ...
  // get inline matrice size which contains 3 channels RGB
  size_t size = rows * cols * 3;
  // loop through all pixels in all channel and set the value to pnm_maxval
  while (size--)
    *p++ = pnm_maxval;
  pnm_save(imd, PnmRawPpm, "a.ppm");

  // cut the image into 4 portions in both horizental and vertical direction
  // i0 the starting position in row, i1 the ending postion in row
  // j0 the starting position in col, j1 the ending position in col
  ...
  for (size_t i = i0; i < i1; i++)
  {
    ...
    {
      p = data + pnm_offset(imd, i, j); // get the red component 
      // p  = data + (i*(cols*3)+j*3);
      *p = pnm_maxval - *p; // remove red component from current 'pixel'
      p++;
    }
  }
